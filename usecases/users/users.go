...
func (ser *Service) SignUp(input entities.User) error {
...
func (ser *Service) SignIn(input entities.User) (token string, err error) {
	details, err := ser.Repository.GetUser(input.Username)
	if err != nil {
		return token, err
	}
	...
	if details.Username == "" {
		return token, entities.ErrUserDoesNotExist
	}

	if !(ComparePasswords(details.Password, input.Password)) {
		return token, entities.ErrInvalidPassword
	}
...
func (ser *Service) SayHello(input entities.User) (message string, err error) {
	details, err := ser.Repository.GetUser(input.Username)
	if err != nil {
		ser.Logger.Error().
